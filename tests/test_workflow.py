...
def test_place_order_workflow(client, mock_response, mocker):
    """Test placing an order via User object."""
    ...
    order = user.place_order(
        market, action=Action.BUY, side=Side.YES, count=5, price=50
    )
    ...
    assert body["count"] == 5
    # Client order ID is optional and not currently auto-generated by the library
...
def test_market_orderbook_workflow(client, mock_response, mocker):
    ...
    # 1. client.get_market("KXTEST") -> GET /markets/KXTEST
    # 2. market.get_orderbook() -> GET /markets/KXTEST/orderbook
